    ...
    public static func from(_ pinnedVersion: PinnedVersion) -> Result<SemanticVersion, ScannableError> {
        ...
        return self.from(scanner).flatMap { version in
            if scanner.isAtEnd {
                ...
                return .failure(ScannableError(message: "syntax of version \"\(version)\" is unsupported",
...
extension SemanticVersion: Scannable {
    ...
    public static func from(_ scanner: Scanner) -> Result<SemanticVersion, ScannableError> {
        ...
        guard let versionBuffer = scanner.scanCharacters(from: versionCharacterSet),
        ...
        guard components.count <= 3 else {
            return .failure(ScannableError(message: "found more than 3 dot-separated components in version",
                                           currentLine: scanner.currentLine))
        }
...
extension Scanner {
    ...
    fileprivate func scanStringWithPrefix(_ prefix: Character, until: String) -> String? {
        ...
        guard let buffer = scanUpToString(until.isEmpty ? "\n" : until),
              buffer.first == prefix else {
            return nil
        }
...
public enum VersionSpecifier: Hashable {
    ...
    public func isSatisfied(by version: PinnedVersion) -> Bool {
        func withSemanticVersion(_ predicate: (SemanticVersion) -> Bool) -> Bool {
            if let semanticVersion = try? SemanticVersion.from(version).get() {
...
public func intersection(_ lhs: VersionSpecifier,
                         _ rhs: VersionSpecifier) -> VersionSpecifier? { // swiftlint:disable:this cyclomatic_complexity
    switch (lhs, rhs) {
